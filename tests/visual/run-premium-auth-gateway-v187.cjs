    ...
    for(const scenario of cases){
      ...
      const result=await page.evaluate(({mode,lang})=>{
        ...
        return {
          ...
          frame:box('.ta-auth-frame'),aside:box('.ta-auth-aside'),main:box('.ta-auth-main'),card:box('.ta-auth-card'),
          languageButton:box('.ta-auth-language'),heading:box('.ta-auth-card-header'),
          headingTextRects:textRects(['.ta-auth-card-header>span','.ta-auth-card-header h2','.ta-auth-card-header p']),
      ...
      if(mobile){
        if(result.aside&&result.aside.display!=='none')failures.push(`${p}: desktop auth aside should collapse below 900px`);
        for(const [name,b] of [['Google',result.google],['signin tab',result.signinTab],['create tab',result.createTab],['primary',result.primary]]){
          if(b&&b.height<36-.25)failures.push(`${p}: ${name} target ${b.height}px (<36px)`);
        }
        if(scenario.width<=600&&result.primary&&result.primary.height<44-.25)failures.push(`${p}: phone primary target ${result.primary.height}px (<44px)`);
      }else{
