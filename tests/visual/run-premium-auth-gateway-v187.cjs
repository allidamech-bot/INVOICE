    ...
    for(const scenario of cases){
      ...
      const result=await page.evaluate(({mode,lang})=>{
        ...
        return {
          ...
          languageButton:box('.premium-auth-language'),heading:box('.auth-card-heading'),tabs:box('.account-entry-tabs'),
          signinTab:box('#signin-tab'),createTab:box('#create-tab'),primary:box('.premium-auth-primary'),forgot:box('.account-forgot:not([hidden])'),
          confirm:box('#confirm-field:not([hidden])'),security:box('.auth-card-security'),fields,
      ...
      if(intersects(result.languageButton,result.heading))failures.push(`${p}: language switch overlaps heading`);
