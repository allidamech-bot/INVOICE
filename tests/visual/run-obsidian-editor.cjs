  ...
  try{
   ...
   const auditDesign=async()=>{
    const issues=await page.evaluate(()=>{
     ...
     if(panelCss.backgroundColor!=='rgb(14, 14, 14)')issues.push('design panel outside canonical workspace: '+panelCss.backgroundColor);
   ...
   if(viewport.width<=1180){
    ...
    assert.equal(preview.background,'rgb(14, 14, 14)');assert.ok(preview.paper>0);
   ...
   }else assert.equal(await page.locator('.preview-stage').evaluate(el=>getComputedStyle(el).backgroundColor),'rgb(14, 14, 14)');
