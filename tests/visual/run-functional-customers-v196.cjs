...
async function runLanguage(browser,lang){
  ...
  assert.deepEqual(calls,['customer-v196:proforma'],`${lang}: list quote action must be single-flight`);
  ...
  assert.deepEqual(calls,['customer-v196:proforma','customer-v196:invoice'],`${lang}: profile invoice action must be single-flight`);
  await page.close();
}

(async()=>{
  const browser=await chromium.launch({headless:true});
  try{await runLanguage(browser,'en');await runLanguage(browser,'ar');console.log('Functional customers v196: 4/4 actions passed');}
