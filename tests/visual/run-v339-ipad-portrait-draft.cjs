...
(async()=>{
  ...
  const context=await browser.newContext({viewport:{width:820,height:1180},userAgent:desktopSafariUa,hasTouch:true});
  ...
  const url=`${BASE}/tests/visual/obsidian-draft-editor.html?lang=en&v=339-portrait`;
  ...
  if(initial.width!==820||initial.height!==1180)failures.push(`portrait viewport=${initial.width}x${initial.height}, expected 820x1180`);
  ...
  if(!['auto','scroll'].includes(initial.mainOverflow))failures.push(`outer .ta-main is not the portrait scroll owner: ${initial.mainOverflow}`);
  if(initial.nestedOverflow!=='visible')failures.push(`nested Draft scroller still owns portrait overflow: ${initial.nestedOverflow}`);
  if(initial.mainScrollHeight<=initial.mainClientHeight)failures.push(`portrait Draft has no real outer scroll range: ${initial.mainScrollHeight}/${initial.mainClientHeight}`);
  ...
  if(!after.editorPresent)failures.push('Draft editor disappeared after portrait scroll');
  if(after.maxScroll>8&&after.scrollTop<after.maxScroll-8)failures.push(`portrait outer scroll did not reach end: ${after.scrollTop}/${after.maxScroll}`);
  if(after.lastBlockBottom>after.viewportHeight+48)failures.push(`last Draft block remained below reachable viewport: ${after.lastBlockBottom}/${after.viewportHeight}`);
  if(page.url()!==initialUrl)failures.push(`URL changed during Draft portrait scroll: ${initialUrl} -> ${page.url()}`);
  if(runtimeErrors.length)failures.push(...runtimeErrors.map(error=>`pageerror: ${error}`));

  await context.close();
  await browser.close();
  if(failures.length){console.error(failures.join('\n'));process.exit(1);}
  console.log('v339 iPad portrait Draft: 820x1180 Desktop-UA WebKit end-scroll stability passed.');
