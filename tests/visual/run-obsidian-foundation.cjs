...
(async()=>{
  ...
  try{
    for(const scenario of scenarios)for(const lang of ['en','ar']){
      ...
      for(const target of base.targets.filter(t=>['#customer','#reference','#currency','#notes'].includes(t.selector))){if(target.background!=='rgb(16, 16, 16)')failures.push(`${target.selector}: incorrect matte input surface ${target.background}`);}
      if(base.canvas!=='rgb(8, 8, 8)')failures.push(`Canvas: ${base.canvas}`);
