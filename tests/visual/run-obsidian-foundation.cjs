...
(async()=>{
  ...
  try{
    for(const scenario of scenarios)for(const lang of ['en','ar']){
      ...
      const focused=(await inspect()).targets.find(t=>t.selector==='#customer');
      if(focused.outline==='none'&&(focused.boxShadow==='none'||!focused.boxShadow))failures.push('Missing keyboard focus indicator');
