...
(async()=>{
 ...
 try{for(const viewport of viewports)for(const lang of ['en','ar']){
  ...
  try{
   ...
   const tabs=page.locator('.settings-tabs>button');assert.equal(await tabs.count(),4,'settings must expose four canonical sections');
   const states=['company','commercial','documents','security'];
