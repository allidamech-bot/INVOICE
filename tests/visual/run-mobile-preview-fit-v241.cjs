...
(async()=>{
      ...
      for(const width of widths)for(const lang of ['en','ar'])for(const kind of ['invoice','proforma']){
        ...
        try{
          ...
          const geometry=await page.locator('.mobile-preview-stage').evaluate(stage=>{
            ...
            const items=paper.querySelector('.items-wrap');
            const finalDetails=paper.querySelector('.final-details');
            ...
            const ir=items?.getBoundingClientRect()||null;
            const dr=finalDetails?.getBoundingClientRect()||null;
            ...
            const itemsToClosingGap=ir&&dr?Math.max(0,(dr.top-ir.bottom)/scale):0;
          ...
          assert.ok(geometry.itemsToClosingGap<=48,'item table and commercial close split inside mobile preview: '+JSON.stringify({width,geometry}));
  ...
  console.log(`Mobile preview v337: ${results.length} Chromium/WebKit phone/language/document cases passed with no A4 dead zone and 44px actions.`);
