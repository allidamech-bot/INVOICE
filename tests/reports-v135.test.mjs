...
test('v135 ships reports navigation print CSV and offline assets without combining currencies',async()=>{
  ...
  assert.ok(css.includes('printing-financial-report'));assert.ok(css.includes('@media print'));assert.ok(css.includes('grid-template-columns:repeat(6,minmax(0,1fr))'),'all six mobile workspaces remain visible');
});
