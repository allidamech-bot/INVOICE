...
test('cloud account protects account actions while an explicit account operation is active',async()=>{
  ...
  assert.match(source,/private signOut=async\(\)=>\{[\s\S]*?if\(this\.operationRunning\)return;[\s\S]*?await this\.props\.onSignOut\(\);[\s\S]*?await suspendSession\(\);[\s\S]*?window\.location\.reload\(\);/);
