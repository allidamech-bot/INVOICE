...
test('new artwork intent supersedes older processing, including artwork mode changes',async()=>{
  const source=await read('src/components/SettingsModal.tsx');
  ...
  assert.match(upload,/cleaningAssets:false,processingAsset:null,error:t\('Image is too large/);
  assert.match(upload,/cleaningAssets:false,processingAsset:null,error:t\('Use a PNG, WebP, or JPEG image/);
  ...
  assert.match(mode,/private setSignatureMode=/);
  assert.match(mode,/private setStampMode=/);
});
