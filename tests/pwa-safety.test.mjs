...
test('service worker never cache-firsts arbitrary cross-origin API traffic',async()=>{
  ...
  assert.match(sw,/EXTERNAL_CORE/);
  assert.match(sw,/url\.origin !== self\.location\.origin/);
  assert.match(sw,/if \(!EXTERNAL_CORE\.has\(url\.href\)\) return/);
  assert.doesNotMatch(sw,/firebaseapis\.com|googleapis\.com|identitytoolkit/);
});

test('essential matched React runtime remains explicitly available offline',async()=>{
  const sw=await read('public/sw.js');
