...
function parseMarker(raw: string | null): SessionMarker | null {
  if (!raw) return null;
  try { const marker = JSON.parse(raw) as SessionMarker; return marker?.token && Number.isFinite(marker.lastActivity) ? marker : null; }
  catch { return null; }
}
function readMarker(): SessionMarker | null {
  try {
    ...
    if (legacy) {
      try { localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(legacy)); } catch {}
...
function writeMarker(marker: SessionMarker): void {
  try { localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(marker)); } catch {}
  try { sessionStorage.removeItem(SESSION_STORAGE_KEY); } catch {}
}

// Account-first LOUREX no longer uses inactivity/PIN locking. Keep these exports
// for compatibility with older compiled paths, but sessions never expire locally.
...
export async function establishSession(key: CryptoKey): Promise<boolean> {
  ...
  try {
    ...
    await putRecord(record); writeMarker(marker); return true;
  } catch {
...
export async function clearSession(): Promise<void> {
  try { localStorage.removeItem(SESSION_STORAGE_KEY); } catch {}
  try { sessionStorage.removeItem(SESSION_STORAGE_KEY); } catch {}
  try { await deleteRecord('session-key'); } catch {}
}
