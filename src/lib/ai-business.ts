import type { Customer, LourexDocument, PaymentRecord, PurchaseRecord, SavedItem, Supplier, VaultPayload } from '../types.js';
...
type ArchivedSavedItem=SavedItem&{archived?:boolean};
...
function percentChange(current:string,previous:string):string{
  const now=decimalToScaled(current||'0',4),before=decimalToScaled(previous||'0',4);if(before===0n)return'';
  const basisPoints=(now-before)*1_000_000n/(before<0n?-before:before);const sign=basisPoints<0n?'-':'';const abs=basisPoints<0n?-basisPoints:basisPoints;
...
function duplicateKey(item:SavedItem):string{
  const sku=normalizeSavedItemSku(item.sku??'');if(sku)return`sku:${sku}`;
  const en=normalizeSavedItemIdentity(item.descriptionEn);if(en)return`en:${en}`;
  const ar=normalizeSavedItemIdentity(item.descriptionAr);return ar?`ar:${ar}`:'';
}
function productCostHistory(purchases:PurchaseRecord[]):Map<string,Array<{currency:string;unitCost:string;landedUnitCost:string;date:string;supplierId:string;supplierName:string}>>{
  ...
  for(const purchase of ordered){
    const supplierId=purchase.supplierSnapshot?.sourceSupplierId||'';const name=supplierName(purchase.supplierSnapshot);
    for(const line of purchase.items){
      if(!line.savedItemId||!line.unitCost.trim())continue;
      const list=map.get(line.savedItemId)??[];
      list.push({currency:purchase.currency,unitCost:line.unitCost,landedUnitCost:line.landedUnitCost||line.unitCost,date:purchase.date,supplierId,supplierName:name});
      map.set(line.savedItemId,list);
    }
  }
  return map;
}
...
export function buildAiBusinessContext(vault:VaultPayload,asOf=todayIso()):AiBusinessContext{
  ...
  for(const item of vault.savedItems){const key=duplicateKey(item);if(!key)continue;const list=duplicateMap.get(key)??[];list.push(item);duplicateMap.set(key,list);}
  ...
  const products:AiProductInsight[]=vault.savedItems.map(raw=>{
    const item=raw as ArchivedSavedItem;const used=isoDate(item.lastUsedAt);const created=isoDate(item.createdAt);const dormant=item.usageCount>0?Boolean(used&&used<=dormantCutoff):Boolean(created&&created<=dormantCutoff);
    const missing:string[]=[];if(!(item.sku??'').trim())missing.push('sku');if(!item.hsCode.trim())missing.push('hsCode');if(!(item.lastUnitCost??'').trim())missing.push('cost');if(!(item.category??'').trim())missing.push('category');
    ...
    const currency=(item.lastCostCurrency||item.lastCurrency||latest?.currency||'USD').toUpperCase();const lastCost=(item.lastUnitCost||latest?.unitCost||'').trim();const suggestedPrice=lastCost?pricingSuggestedUnitPrice(lastCost,vault.company.commercial.pricing):'';
    const signals:string[]=[];if(dormant)signals.push('dormant-90-plus');if(duplicatePairs.has(item.id))signals.push('possible-duplicate');if(missing.length)signals.push('incomplete-product-data');
    if(lastCost&&item.lastUnitPrice&&item.lastCurrency===currency&&decimalToScaled(item.lastUnitPrice,4)<decimalToScaled(lastCost,4))signals.push('sale-price-below-cost');
    if(costChange&&absPercent(costChange)>=2000n)signals.push(decimalToScaled(costChange,2)>0n?'cost-up-20-plus':'cost-down-20-plus');
    if(item.archived)signals.push('archived');
    return{id:item.id,name:itemName(item),sku:item.sku??'',currency,lastSalePrice:item.lastUnitPrice||'',lastCost,suggestedPrice,dormant,archived:Boolean(item.archived),missing,duplicateWith:duplicatePairs.get(item.id)||'',costChangePercent:costChange,signals};
  }).sort((a,b)=>Number(b.signals.length>0)-Number(a.signals.length>0)||b.signals.length-a.signals.length||a.name.localeCompare(b.name)).slice(0,20);

  const actions:AiActionCandidate[]=[];
  for(const row of products){
    if(row.archived){actions.push({type:'item.restore',itemId:row.id,relatedItemId:'',label:`Restore ${row.name}`,reason:'Product is currently archived.'});continue;}
    if(row.duplicateWith)actions.push({type:'item.review-duplicate',itemId:row.id,relatedItemId:row.duplicateWith,label:`Review duplicate ${row.name}`,reason:'A matching SKU or normalized product name exists.'});
    if(row.missing.length)actions.push({type:'item.complete-data',itemId:row.id,relatedItemId:'',label:`Complete ${row.name}`,reason:`Missing: ${row.missing.join(', ')}.`});
    if(row.dormant)actions.push({type:'item.archive',itemId:row.id,relatedItemId:'',label:`Archive ${row.name}`,reason:'No recorded use for at least 90 days.'});
  }
  ...
  for(const purchase of vault.purchases){
    if(purchase.status!=='posted'||!purchaseAccountingIsValid(purchase))continue;
    const supplierId=purchase.supplierSnapshot?.sourceSupplierId||'';const name=supplierName(purchase.supplierSnapshot);const key=`${supplierId||name}|${purchase.currency}`;const row=supplierRowsMap.get(key)??{supplierId,supplierName:name,currency:purchase.currency,count:0,landed:0n,lastDate:''};
    row.count+=1;row.landed+=decimalToScaled(purchaseTotals(purchase).landedTotal,2);if(purchase.date>row.lastDate)row.lastDate=purchase.date;supplierRowsMap.set(key,row);
  }
  ...
  for(const [itemId,rows] of history){
    const saved=vault.savedItems.find(item=>item.id===itemId);if(!saved)continue;
    const grouped=new Map<string,typeof rows[number]>();
    for(const row of rows){const key=`${row.supplierId||row.supplierName}|${row.currency}`;if(!grouped.has(key))grouped.set(key,row);}
    const byCurrency=new Map<string,typeof rows>();for(const row of grouped.values()){const list=byCurrency.get(row.currency)??[];list.push(row);byCurrency.set(row.currency,list);}
    for(const [currency,observed] of byCurrency){
      const valid=observed.filter(row=>row.unitCost.trim()).sort((a,b)=>Number(decimalToScaled(a.unitCost,4)-decimalToScaled(b.unitCost,4)));
      if(valid.length>=2){const best=valid[0]!;itemComparisons.push({itemId,itemName:itemName(saved),currency,observations:valid.slice(0,4).map(row=>({supplierId:row.supplierId,supplierName:row.supplierName,unitCost:row.unitCost,landedUnitCost:row.landedUnitCost,purchaseDate:row.date})),lowestObservedSupplierId:best.supplierId,lowestObservedSupplierName:best.supplierName,lowestObservedUnitCost:best.unitCost});}
    }
    const latest=rows[0];if(latest){const previous=rows.find(row=>row.currency===latest.currency&&row.date<=latest.date&&row.unitCost!==latest.unitCost);if(previous){const change=percentChange(latest.unitCost,previous.unitCost);if(change&&absPercent(change)>=2000n)costAlerts.push({itemId,itemName:itemName(saved),currency:latest.currency,supplierName:latest.supplierName,currentUnitCost:latest.unitCost,previousUnitCost:previous.unitCost,changePercent:change,direction:decimalToScaled(change,2)>0n?'up':'down'});}}
  }
  itemComparisons.sort((a,b)=>a.itemName.localeCompare(b.itemName));costAlerts.sort((a,b)=>absPercent(b.changePercent)>absPercent(a.changePercent)?1:-1);
  ...
  const customerRows:AiCustomerInsight[]=receivables.map(receivable=>{
    const customer=receivable.customer;const customerId=receivable.customerId;const invoices=vault.documents.filter(doc=>activeInvoiceForCustomer(doc,customerId));
    const paidSamples:Array<{daysToPay:number;daysLate:number}>=[];
    for(const invoice of invoices){
      const summary=invoicePaymentSummary(invoice,vault.payments,asOf,vault.documents);if(summary.status!=='paid')continue;
      const linked=vault.payments.filter(payment=>payment.invoiceId===invoice.id&&payment.date<=asOf).sort((a,b)=>a.date.localeCompare(b.date));const last=linked.at(-1);if(!last)continue;
      paidSamples.push({daysToPay:daysBetween(invoice.issueDate,last.date),daysLate:invoice.dueDate&&last.date>invoice.dueDate?daysBetween(invoice.dueDate,last.date):0});
    }
    const profitMap=new Map<string,{revenue:bigint;profit:bigint;complete:boolean;missing:number}>();
    for(const doc of vault.documents.filter(doc=>doc.kind==='invoice'&&doc.status==='final'&&doc.lifecycleStatus!=='voided'&&receivableCustomerId(doc)===customerId)){
      const profit=calculateProfitability(doc);const row=profitMap.get(doc.currency)??{revenue:0n,profit:0n,complete:true,missing:0};row.revenue+=decimalToScaled(profit.netRevenue,2);row.missing+=profit.missingCostItems;if(!profit.complete)row.complete=false;else row.profit+=decimalToScaled(profit.grossProfit,2);profitMap.set(doc.currency,row);
    }
    ...
    const riskSignals:string[]=[];if(receivable.hasOverdue)riskSignals.push('overdue-balance');if(receivable.currencies.some(row=>decimalToScaled(row.aging.days90plus,2)>0n))riskSignals.push('aging-90-plus');
    const avgLate=paidSamples.length?Math.round(paidSamples.reduce((sum,row)=>sum+row.daysLate,0)/paidSamples.length):null;if(avgLate!==null&&avgLate>=15)riskSignals.push('historically-late-payments');
    if(customer?.creditLimit&&customer.creditCurrency){const row=receivable.currencies.find(entry=>entry.currency===customer.creditCurrency);if(row&&decimalToScaled(customer.creditLimit,2)>0n&&decimalToScaled(row.outstanding,2)>decimalToScaled(customer.creditLimit,2))riskSignals.push('credit-limit-exceeded');}
    const lastDoc=invoices.reduce((latest,doc)=>maxStamp(latest,doc.updatedAt),''),lastPayment=vault.payments.filter(payment=>payment.customerId===customerId).reduce((latest,payment)=>maxStamp(latest,payment.updatedAt||payment.createdAt),''),lastActivity=maxStamp(lastDoc,lastPayment,customer?.updatedAt||'');
    const priority:riskSignals.includes('aging-90-plus')||riskSignals.includes('credit-limit-exceeded')?'high':riskSignals.length?'medium':'normal';
  ...
  return{
    version:1,basis:'deterministic-business-intelligence',asOf,
    daily,
    actionCenter:{candidates:actions.slice(0,16),duplicateGroups,dormantCandidates:products.filter(row=>row.dormant&&!row.archived).length,missingDataCandidates:products.filter(row=>row.missing.length>0&&!row.archived).length,archivedItems:products.filter(row=>row.archived).length},
    products:{rows:products},
    suppliers:{rows:supplierRows,itemComparisons:itemComparisons.slice(0,12),costAlerts:costAlerts.slice(0,12)},
    customers:{rows:customerRows},
    limitations:['currencies-remain-separate','pricing-suggestions-use-saved-cost-and-company-policy','supplier-comparison-is-observed-history-not-a-purchase-recommendation','customer-risk-signals-are-rule-based-not-credit-scoring','ai-may-explain-deterministic-results-but-must-not-recalculate-accounting']
  };
}
