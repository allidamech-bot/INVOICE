...
const FOLLOW_UP_PRIORITY_RANK:Record<AiCustomerInsight['followUpPriority'],number>={high:2,medium:1,normal:0};
...
function percentChange(current:string,previous:string):string{const now=decimalToScaled(current||'0',COST_DECIMALS),before=decimalToScaled(previous||'0',COST_DECIMALS);if(before===0n)return'';const basisPoints=(now-before)*1_000_000n/(before<0n?-before:before);const sign=basisPoints<0n?'-':'';const abs=basisPoints<0n?-basisPoints:basisPoints;return `${sign}${abs/10_000n}.${((abs%10_000n)/100n).toString().padStart(2,'0')}`;}
...
export function buildAiBusinessContext(vault:VaultPayload,asOf=todayIso()):AiBusinessContext{
  ...
  const customerRows:AiCustomerInsight[]=receivables.map(receivable=>{
    ...
    const lastDoc=invoices.reduce((latest,doc)=>maxStamp(latest,doc.updatedAt),''),lastPayment=vault.payments.filter(payment=>payment.customerId===customerId).reduce((latest,payment)=>maxStamp(latest,payment.updatedAt||payment.createdAt),''),lastActivity=maxStamp(lastDoc,lastPayment,customer?.updatedAt||'');const priority:AiCustomerInsight['followUpPriority']=riskSignals.includes('aging-90-plus')||riskSignals.includes('credit-limit-exceeded')?'high':riskSignals.length?'medium':'normal';
  ...
  }).sort((a,b)=>FOLLOW_UP_PRIORITY_RANK[b.followUpPriority]-FOLLOW_UP_PRIORITY_RANK[a.followUpPriority]||b.lastActivity.localeCompare(a.lastActivity)).slice(0,16);
