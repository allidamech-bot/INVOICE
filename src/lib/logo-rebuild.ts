 ...
 * tests. The user-facing "recreate" workflow below is interactive because
...
export function rebuildLogoTransparentPixels(data:Uint8ClampedArray,width:number,height:number):boolean{
  ...
  for(let pixel=0;pixel<total;pixel+=1){
    ...
    if((keep[pixel]??0)||preserveLabels.has(labels[pixel]??-1)){kept+=1;continue;}
...
function loadImage(src:string):Promise<HTMLImageElement>{
  return new Promise((resolve,reject)=>{
    ...
    image.onerror=()=>reject(new Error(t('Unable to open logo image.','تعذر فتح صورة الشعار.')));
    image.src=src;
  });
}

function hasVisiblePixels(pixels:ImageData):boolean{
  for(let index=3;index<pixels.data.length;index+=4)if((pixels.data[index]??0)>12)return true;
  return false;
}
...
function crop(canvas:HTMLCanvasElement,pixels:ImageData):string{
  ...
  const context=output.getContext('2d');if(!context)return canvas.toDataURL('image/png');
...
async function openManualBackgroundEditor(src:string):Promise<string>{
  const image=await loadImage(src),naturalWidth=image.naturalWidth||image.width,naturalHeight=image.naturalHeight||image.height;
  if(!naturalWidth||!naturalHeight)return src;

  // 1024px is ample for an invoice logo and avoids iOS Safari memory pressure.
  // A 1600px square RGBA snapshot is ~10 MB; multiple undo snapshots could
  // previously exceed 100 MB before Canvas/browser buffers were counted.
  ...
  heading.append(title,span(t('Tap an unwanted area to remove it. Use the eraser when the unwanted pixels touch the logo edge.','اضغط على الجزء غير المرغوب لحذفه. استخدم الممحاة عندما تكون البقايا ملاصقة لحافة الشعار.'),'logo-touch-editor-subtitle'));
  ...
  const undoButton=button(t('Undo','تراجع'),'');undoButton.disabled=true;const resetButton=button(t('Reset','إعادة ضبط'),'');utility.append(undoButton,resetButton);controls.appendChild(utility);sheet.appendChild(controls);
  ...
  const coordinates=(event:PointerEvent)=>{
    ...
    const x=Math.max(0,Math.min(width-1,Math.floor((event.clientX-rect.left)*width/Math.max(1,rect.width))));
    const y=Math.max(0,Math.min(height-1,Math.floor((event.clientY-rect.top)*height/Math.max(1,rect.height))));
    return{x,y};
  };

  // Erase only the brush rectangle. The old implementation copied the entire
  // canvas on every pointermove, which was especially expensive on iPhone.
  const eraseAt=(x:number,y:number):boolean=>{
    ...
    const localWidth=right-left+1,localHeight=bottom-top+1,pixels=context.getImageData(left,top,localWidth,localHeight),data=pixels.data;
    let changed=false;
    for(let ly=0;ly<localHeight;ly+=1)for(let lx=0;lx<localWidth;lx+=1){
      const px=left+lx,py=top+ly,dx=px-x,dy=py-y;if(dx*dx+dy*dy>r2)continue;
      const alphaIndex=(ly*localWidth+lx)*4+3;if((data[alphaIndex]??0)<=0)continue;data[alphaIndex]=0;changed=true;
    }
    if(changed)context.putImageData(pixels,left,top);
    return changed;
  };

  const floodAt=(x:number,y:number):boolean=>{
    const pixels=context.getImageData(0,0,width,height),data=pixels.data,total=width*height,seedPixel=y*width+x,seed=seedPixel*4;
    if((data[seed+3]??0)<=10)return false;
    const sr=data[seed]??0,sg=data[seed+1]??0,sb=data[seed+2]??0,threshold=tolerance*tolerance*3,visited=new Uint8Array(total),queue=new Int32Array(total);
    let start=0,end=0,changed=false;queue[end++]=seedPixel;visited[seedPixel]=1;
    while(start<end){
  ...
  const setTool=(next:'tap'|'erase')=>{
    tool=next;tapButton.classList.toggle('active',tool==='tap');eraseButton.classList.toggle('active',tool==='erase');
    slider.min='8';slider.max=tool==='tap'?'90':'72';slider.value=String(tool==='tap'?tolerance:brush);
    sliderText.textContent=tool==='tap'?`${t('Color tolerance','حساسية اللون')}: ${tolerance}`:`${t('Eraser size','حجم الممحاة')}: ${brush}`;
  };
  ...
  resetButton.onclick=()=>{context.putImageData(new ImageData(new Uint8ClampedArray(original.data),original.width,original.height),0,0);history.length=0;updateUndo();hint.textContent=t('Original image restored.','تمت استعادة الصورة الأصلية.');};
  ...
  canvas.onpointerdown=(event)=>{
    event.preventDefault();canvas.setPointerCapture?.(event.pointerId);const{x,y}=coordinates(event);snapshot();
    if(tool==='tap'){if(!floodAt(x,y))discardLastSnapshot();return;}
    drawing=true;if(!eraseAt(x,y))discardLastSnapshot();
  };
  ...
  return await new Promise<string>(resolve=>{
    ...
    const onKeyDown=(event:KeyboardEvent)=>{if(event.key==='Escape'){event.preventDefault();finish(src);}};
    ...
    applyButton.onclick=()=>{
      ...
      if(!hasVisiblePixels(pixels)){hint.textContent=t('The logo cannot be empty. Undo or reset before applying.','لا يمكن اعتماد شعار فارغ. استخدم تراجع أو إعادة ضبط أولًا.');return;}
      finish(crop(canvas,pixels));
    };
  });
}
