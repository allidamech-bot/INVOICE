...
function moneyFromScaled4(value:bigint):string{const negative=value<0n;const abs=negative?-value:value;const cents=(abs+50n)/100n;return `${negative?'-':''}${cents/100n}.${(cents%100n).toString().padStart(2,'0')}`;}
...
export function buildSupplierPurchasingContext(vault:VaultPayload,message:string,asOf:string):SupplierPurchasingContext{
  ...
  for(const rows of byItemCurrency.values()){
    ...
    const latest=[...latestBySupplier.values()];if(latest.length>=2){const byUnit=[...latest].sort((a,b)=>{const av=decimalToScaled(a.unitCost,4),bv=decimalToScaled(b.unitCost,4);return av<bv?-1:av>bv?1:0;});const byLanded=[...latest].sort((a,b)=>{const av=decimalToScaled(a.landedUnitCost,4),bv=decimalToScaled(b.landedUnitCost,4);return av<bv?-1:av>bv?1:0;});const unit=byUnit[0]!,landed=byLanded[0]!;comparisons.push({itemId:unit.itemId,itemName:unit.itemName,currency:unit.currency,basis:'latest-posted-observation-per-supplier',observations:latest.slice(0,8).map(({supplierId,supplierName,purchaseId,purchaseNumber,purchaseDate,unitCost,landedUnitCost})=>({supplierId,supplierName,purchaseId,purchaseNumber,purchaseDate,unitCost,landedUnitCost})),lowestUnitCostSupplierId:unit.supplierId,lowestUnitCostSupplierName:unit.supplierName,lowestUnitCost:unit.unitCost,lowestLandedCostSupplierId:landed.supplierId,lowestLandedCostSupplierName:landed.supplierName,lowestLandedUnitCost:landed.landedUnitCost});}
    const bySupplier=new Map<string,CostObservation[]>();for(const row of rows){const key=row.supplierId||row.supplierName;const list=bySupplier.get(key)??[];list.push(row);bySupplier.set(key,list);}for(const supplierRows of bySupplier.values()){if(supplierRows.length<2)continue;const current=supplierRows[0]!,previous=supplierRows.find(row=>row.unitCost!==current.unitCost||row.landedUnitCost!==current.landedUnitCost);if(!previous)continue;const unitChange=percent(current.unitCost,previous.unitCost),landedChange=percent(current.landedUnitCost,previous.landedUnitCost);if(!unitChange&&!landedChange)continue;const direction=decimalToScaled(unitChange||landedChange,2)>=0n?'up':'down';const trend:SupplierCostTrend={itemId:current.itemId,itemName:current.itemName,supplierId:current.supplierId,supplierName:current.supplierName,currency:current.currency,currentDate:current.purchaseDate,previousDate:previous.purchaseDate,currentUnitCost:current.unitCost,previousUnitCost:previous.unitCost,currentLandedUnitCost:current.landedUnitCost,previousLandedUnitCost:previous.landedUnitCost,unitCostChangePercent:unitChange,landedCostChangePercent:landedChange,direction};trends.push(trend);if(unitChange&&absolutePercent(unitChange)>=2000n)alerts.push({type:'cost-change',purchaseId:current.purchaseId,purchaseNumber:current.purchaseNumber,supplierName:current.supplierName,itemId:current.itemId,itemName:current.itemName,currency:current.currency,value:unitChange,threshold:'20.00',detail:`Unit cost changed ${unitChange}% versus the previous posted observation from this supplier.`});}}
  }
