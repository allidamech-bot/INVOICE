...
function scaledCostToMoney(value:bigint):string{const sign=value<0n?-1n:1n;const abs=value<0n?-value:value;const cents=(abs+COST_TO_CENTS/2n)/COST_TO_CENTS;return centsString(sign*cents);}
...
function markupPercent(price:string,cost:string):string{const p=decimalToScaled(price||'0',COST_DECIMALS),c=decimalToScaled(cost||'0',COST_DECIMALS);return c>0n?percentString(p-c,c):'';}
function marginPercent(price:string,cost:string):string{const p=decimalToScaled(price||'0',COST_DECIMALS),c=decimalToScaled(cost||'0',COST_DECIMALS);return p>0n?percentString(p-c,p):'';}
...
function scenarioFor(cost:string,rounding:string,request:PricingScenarioRequest|null):PricingScenarioResult|null{
  if(!cost||!request)return null;const unitPrice=pricingSuggestedUnitPrice(cost,{method:request.method,percent:request.percent,rounding});if(!unitPrice)return null;const p=decimalToScaled(unitPrice,COST_DECIMALS),c=decimalToScaled(cost,COST_DECIMALS);return{method:request.method,percent:request.percent,unitPrice,grossProfit:scaledCostToMoney(p-c),marginPercent:marginPercent(unitPrice,cost),markupPercent:markupPercent(unitPrice,cost)};
}
...
export function buildProductPricingContext(vault:VaultPayload,message:string,asOf:string):ProductPricingContext{
  ...
  const rows=vault.savedItems.filter(item=>!item.archived).map(item=>{
    ...
    let pricingHealth:PricingHealth='meets-policy';if(!cost)pricingHealth='no-cost';else if(!salePrice)pricingHealth='no-sale-price';else if(!comparable)pricingHealth='currency-mismatch';else if(decimalToScaled(salePrice,COST_DECIMALS)<decimalToScaled(cost,COST_DECIMALS))pricingHealth='below-cost';else if(suggestedPrice&&decimalToScaled(salePrice,COST_DECIMALS)<decimalToScaled(suggestedPrice,COST_DECIMALS))pricingHealth='below-policy';
