...
export async function requestProductImportAiMapping(analysis:ProductImportAnalysis,mapping:ProductImportColumnMap):Promise<ProductImportAiResult>{
  const columns=ambiguousProductImportColumns(analysis,mapping);
  if(!columns.length)return {model:'local',mappings:[]};
  const response=await fetch('/api/product-import-ai',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'LOUREX-Invoice'},body:JSON.stringify({columns})});
...
export function mergeProductImportAiMapping(analysis:ProductImportAnalysis,current:ProductImportColumnMap,suggestions:ProductImportAiSuggestion[]):ProductImportColumnMap{
  ...
  for(const suggestion of suggestions){
    ...
    for(let index=0;index<next.length;index+=1){if(index!==suggestion.index&&next[index]===suggestion.field&&analysis.columns[index]?.confidence!=='high')next[index]=null;}
