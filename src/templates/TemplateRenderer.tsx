...
function PartyBlock({ document: doc, type }: { document: LourexDocument; type: 'seller' | 'customer' }): any {
  ...
  return <section className={`party-block party-${type}`}><div className="section-kicker">{localized(doc, isSeller ? 'Seller / From' : 'Buyer / Customer', isSeller ? 'البائع / من' : 'المشتري / العميل')}</div><div className="party-name">{name}</div>{(addressEn || addressAr) ? <div className="party-address">{valuePair(doc, addressEn, addressAr)}</div> : null}{(city || country) ? <div>{[city, country].filter(Boolean).join(', ')}</div> : null}{(phone || email || website) ? <div className="party-contact">{[phone, email, website].filter(Boolean).join(' • ')}</div> : null}{visibleIdentifiers.length?<div className="party-identifiers">{visibleIdentifiers.map(([en,ar,value])=><div key={`${en}-${value}`}><b>{localized(doc,en,ar)}</b><span>{value}</span></div>)}</div>:null}</section>;
}
...
function Page({ document: doc, items, pageIndex, totalPages, finalPage, variant, compact }: PageProps): any {
  ...
  const darkTone=FULL_DARK_TEMPLATES.includes(variant);
