...
function Totals({ document: doc }: { document: LourexDocument }): any {
  ...
  return <section className="totals-block">{rows.filter(r => r[3]).map(r => <div className="total-row" data-total={r[0].toLowerCase().replaceAll(' ','-')} key={r[0]}><span>{localized(doc,r[0],r[1])}</span><strong>{formatMoney(r[2],currency)}</strong></div>)}<div className="grand-total"><span>{localized(doc,doc.role==='credit-note'?'Credit Total':doc.kind==='purchase-order'?'Order Total':'Grand Total',doc.role==='credit-note'?'إجمالي الإشعار الدائن':doc.kind==='purchase-order'?'إجمالي الطلب':'الإجمالي النهائي')}</span><strong>{formatMoney(t.grandTotal,currency)}</strong></div></section>;
}
function Bank({ document: doc }: { document: LourexDocument }): any {
  if (!documentBankAllowed(doc.kind,doc.role) || !doc.appearance.showBank) return null; const b = doc.companySnapshot.bank; const rawRows = ([['Bank Name','اسم البنك',b.bankName,'neutral'],['Account Name','اسم الحساب',b.accountName,'neutral'],['IBAN','آيبان',b.iban,'neutral'],['SWIFT / BIC','سويفت',b.swift,'neutral'],['Currency','العملة',b.currency,'currency']] as Array<[string,string,string,DocumentValueKind]>);const rows=rawRows.map(([en,ar,value,kind])=>[en,ar,safeValue(doc,value,kind)] as [string,string,string]).filter((r): r is [string,string,string] => Boolean(r[2]?.trim())); if (!rows.length) return null;
