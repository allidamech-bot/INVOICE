...
function Page({ document: doc, items, pageIndex, totalPages, finalPage, variant, compact }: PageProps): any {
  ...
  return <article className={`invoice-page template-${variant} lang-${doc.language} palette-${paletteMode} ${compact ? 'compact-preview' : ''}`} data-palette={paletteMode} style={style}><div className="page-accent" />{variant === 'executive' ? <header className="header-executive"><LogoBlock document={doc} inverse={true}/><div className="doc-title inverse"><span>{typeEn}</span>{doc.language !== 'en' ? <em dir="rtl">{typeAr}</em> : null}</div><MetaBlock document={doc}/></header> : null}{variant === 'minimal' ? <header className="header-minimal"><LogoBlock document={doc}/><div><div className="doc-title"><span>{typeEn}</span>{doc.language !== 'en' ? <em dir="rtl">{typeAr}</em> : null}</div><MetaBlock document={doc}/></div></header> : null}{variant === 'trade' ? <header className="header-trade"><div className="trade-bar"><LogoBlock document={doc} inverse={true}/></div><div className="trade-title"><div className="doc-title"><span>{typeEn}</span>{doc.language !== 'en' ? <em dir="rtl">{typeAr}</em> : null}</div><MetaBlock document={doc}/></div></header> : null}{variant === 'signature' ? <header className="header-signature"><LogoBlock document={doc}/><div className="signature-title"><small>{localized(doc,'COMMERCIAL DOCUMENT','مستند تجاري')}</small><div className="doc-title"><span>{typeEn}</span>{doc.language !== 'en' ? <em dir="rtl">{typeAr}</em> : null}</div></div><MetaBlock document={doc}/></header> : null}{isModernTemplate(variant) ? <ModernHeader document={doc} variant={variant}/> : null}<main className="doc-body">{pageIndex === 0 ? <div className="party-grid"><PartyBlock document={doc} type="seller"/><PartyBlock document={doc} type="customer"/></div> : null}{items.length ? <ItemsTable document={doc} items={items} continued={pageIndex > 0}/> : null}{finalPage ? <FinalDetails document={doc}/> : null}</main><footer className="doc-footer"><span>{doc.companySnapshot.footerText || doc.companySnapshot.nameEn || doc.companySnapshot.nameAr}</span><span>{pageIndex + 1} / {totalPages}</span></footer></article>;
}
