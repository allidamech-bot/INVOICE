...
function MetaBlock({ document: doc }: { document: LourexDocument }): any {
  return <div className="doc-meta"><div className="meta-number"><b>{localized(doc, 'No.', 'الرقم')}</b><span>{doc.number}</span></div>{doc.revision>1?<div className="meta-revision"><b>{localized(doc,'Revision','المراجعة')}</b><span>R{doc.revision}</span></div>:null}{doc.creditForNumber?<div className="meta-source"><b>{localized(doc,'Source Invoice','الفاتورة الأصلية')}</b><span>{doc.creditForNumber}</span></div>:null}<div className="meta-issue"><b>{localized(doc, 'Issue Date', 'تاريخ الإصدار')}</b><span>{displayDate(doc.issueDate, doc.language)}</span></div>{doc.dueDate ? <div className="meta-due"><b>{localized(doc, doc.kind === 'proforma' ? 'Valid Until' : 'Due Date', doc.kind === 'proforma' ? 'صالح حتى' : 'تاريخ الاستحقاق')}</b><span>{displayDate(doc.dueDate, doc.language)}</span></div> : null}<div className="meta-currency"><b>{localized(doc, 'Currency', 'العملة')}</b><span>{doc.currency}</span></div></div>;
}
