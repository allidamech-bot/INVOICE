...
function auth():any{ensureFirebase();return firebase.auth();} function db():any{ensureFirebase();return firebase.firestore();}
...
function writeSyncAnchor(uid:string,meta:Pick<CloudVaultMeta,'revision'|'cipherSha256'|'updatedAt'>):void{try{localStorage.setItem(syncAnchorKey(uid),JSON.stringify({revision:meta.revision,cipherSha256:meta.cipherSha256,updatedAt:meta.updatedAt} satisfies CloudSyncAnchor));}catch{}}
...
async function pruneCloudHistory(uid:string):Promise<void>{try{const snap=await historyCollection(uid).orderBy('updatedAt','desc').get();const docs=snap.docs||[];for(const doc of docs.slice(HISTORY_LIMIT)){const data=doc.data();if(validMeta(data))await cleanupRevision(uid,data.revision,data.chunkCount);try{await doc.ref.delete();}catch{}}}catch{}}
async function commitMetaIfUnchanged(uid:string,meta:CloudVaultMeta,previous:CloudVaultMeta|null):Promise<void>{const ref=vaultCollection(uid).doc('meta');await db().runTransaction(async(transaction:any)=>{const snap=await transaction.get(ref);if(previous){if(!snap.exists)throw new Error('Cloud changed during sync. Use Sync Now and try again.');const current=snap.data();if(!validMeta(current)||current.revision!==previous.revision||current.updatedAt!==previous.updatedAt||current.cipherSha256!==previous.cipherSha256)throw new Error('Cloud changed during sync. Use Sync Now and try again.');}else if(snap.exists)throw new Error('Cloud changed during sync. Use Sync Now and try again.');transaction.set(ref,meta);});}
async function commitSingleChunkIfUnchanged(uid:string,meta:CloudVaultMeta,previous:CloudVaultMeta|null,cipher:string):Promise<void>{
  ...
  const chunkRef=col.doc(`chunk-${meta.revision}-00000`);
  await db().runTransaction(async(transaction:any)=>{
    ...
    if(previous){
      ...
      if(!validMeta(current)||current.revision!==previous.revision||current.updatedAt!==previous.updatedAt||current.cipherSha256!==previous.cipherSha256)throw new Error('Cloud changed during sync. Use Sync Now and try again.');
    }else if(snap.exists)throw new Error('Cloud changed during sync. Use Sync Now and try again.');
    transaction.set(chunkRef,{revision:meta.revision,index:0,data:cipher});
...
export async function pushLocalVaultToCloud(uid:string,localSnapshot?:EncryptedVaultRecord|null):Promise<void>{
  ...
  const [security,storedVault,previous]=await Promise.all([getSecurity(),localSnapshot?Promise.resolve(localSnapshot):getEncryptedVault(),getCloudVaultMeta(uid)]);
  ...
  if(previous){
    ...
    if(!anchor)throw new Error('Cloud safety check stopped this upload because this device has not verified the current cloud version. Use Sync Now before uploading local changes.');
    const remoteChanged=previous.revision!==anchor.revision||previous.cipherSha256!==anchor.cipherSha256;
    if(remoteChanged)throw new Error('Cloud has changes from another device. This device was prevented from overwriting them. Use Sync Now to retrieve the latest cloud data first.');
  }
  ...
  if(previous&&previous.revision!==revision)void pruneCloudHistory(uid);
}
...
export async function reconcileCloudVault(uid:string):Promise<CloudSyncResult>{
  ...
  if(local&&!remote){if(startup){window.setTimeout(()=>void pushLocalVaultToCloud(uid).catch(()=>undefined),1500);return 'same';}await pushLocalVaultToCloud(uid);return 'pushed';}
  ...
  if(anchor){
    ...
    if(localChanged&&remoteChanged)throw new Error('Cloud conflict detected: this device and another device both changed since the last successful sync. Nothing was overwritten. Sync from the device with the newest complete data or export a backup before resolving the conflict.');
  ...
  throw new Error('Cloud safety check found different local and cloud data on a device with no verified sync checkpoint. Nothing was overwritten. Use the device that contains the newest complete data and export a backup before resolving this conflict.');
}
