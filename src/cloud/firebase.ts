...
function ensureFirebase():void{if(typeof firebase==='undefined')throw new Error('Firebase is unavailable. Check your internet connection and reload.');if(!initialized){if(!firebase.apps?.length)firebase.initializeApp(FIREBASE_CONFIG);initialized=true;}}
...
function currentDeviceId():string{try{const saved=localStorage.getItem(deviceIdKey());if(saved&&/^[a-z0-9-]{12,80}$/i.test(saved))return saved;const bytes=new Uint8Array(10);crypto.getRandomValues(bytes);const created=`device-${Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('')}`;localStorage.setItem(deviceIdKey(),created);return created;}catch{return 'device-unknown';}}
function readSyncAnchor(uid:string):CloudSyncAnchor|null{try{const raw=localStorage.getItem(syncAnchorKey(uid));if(!raw)return null;const parsed=JSON.parse(raw);if(!parsed||typeof parsed.revision!=='string'||typeof parsed.cipherSha256!=='string'||!/^[0-9a-f]{64}$/i.test(parsed.cipherSha256)||typeof parsed.updatedAt!=='string')return null;return {revision:parsed.revision,cipherSha256:parsed.cipherSha256,updatedAt:parsed.updatedAt};}catch{return null;}}
function writeSyncAnchor(uid:string,meta:Pick<CloudVaultMeta,'revision'|'cipherSha256'|'updatedAt'>):void{try{localStorage.setItem(syncAnchorKey(uid),JSON.stringify({revision:meta.revision,cipherSha256:meta.cipherSha256,updatedAt:meta.updatedAt} satisfies CloudSyncAnchor));clearConflict(uid);}catch{}}
function readConflict(uid:string):CloudConflictToken|null{try{const raw=localStorage.getItem(conflictKey(uid));if(!raw)return null;const parsed=JSON.parse(raw);if(!parsed||typeof parsed.localHash!=='string'||typeof parsed.remoteRevision!=='string'||typeof parsed.remoteHash!=='string'||typeof parsed.detectedAt!=='string')return null;return parsed as CloudConflictToken;}catch{return null;}}
function writeConflict(uid:string,localHash:string,remote:CloudVaultMeta):void{try{localStorage.setItem(conflictKey(uid),JSON.stringify({localHash,remoteRevision:remote.revision,remoteHash:remote.cipherSha256,detectedAt:new Date().toISOString()} satisfies CloudConflictToken));}catch{}}
...
function validBase64Bytes(value:unknown,minBytes:number,maxBytes:number):value is string{if(typeof value!=='string'||!value||value.length%4!==0||value.length>Math.ceil(maxBytes/3)*4+4||!/^[A-Za-z0-9+/]*={0,2}$/.test(value))return false;try{const bytes=atob(value).length;return bytes>=minBytes&&bytes<=maxBytes;}catch{return false;}}
function validSecurity(value:any):value is SecurityMetadata{return Boolean(value&&value.id==='security'&&value.version===1&&Number.isInteger(value.iterations)&&value.iterations>=10_000&&value.iterations<=2_000_000&&validBase64Bytes(value.salt,16,64)&&validBase64Bytes(value.verifierIv,12,12)&&validBase64Bytes(value.verifierCipher,16,512));}
function validMeta(data:any):data is CloudVaultMeta{return Boolean(data&&data.format===CLOUD_FORMAT&&data.version===1&&typeof data.revision==='string'&&data.revision.length>0&&data.revision.length<160&&typeof data.updatedAt==='string'&&!Number.isNaN(Date.parse(data.updatedAt))&&Number.isInteger(data.schemaVersion)&&data.schemaVersion>0&&data.schemaVersion<100&&validBase64Bytes(data.iv,12,12)&&Number.isInteger(data.cipherLength)&&data.cipherLength>0&&data.cipherLength<=MAX_CIPHER_LENGTH&&typeof data.cipherSha256==='string'&&/^[0-9a-f]{64}$/i.test(data.cipherSha256)&&Number.isInteger(data.chunkCount)&&data.chunkCount>=1&&data.chunkCount<=MAX_CHUNKS&&validSecurity(data.security));}
...
export async function cloudRemoteChangedSinceAnchor(uid:string):Promise<boolean>{requireCurrentUid(uid);const remote=await getCloudVaultMeta(uid);if(!remote)return false;const anchor=readSyncAnchor(uid);if(!anchor)return true;return remote.revision!==anchor.revision||remote.cipherSha256!==anchor.cipherSha256;}

async function writeChunkBatch(uid:string,revision:string,chunks:string[],offset:number):Promise<void>{const col=vaultCollection(uid);const batch=db().batch();chunks.slice(offset,offset+WRITE_BATCH_CHUNKS).forEach((data,index)=>batch.set(col.doc(`chunk-${revision}-${String(offset+index).padStart(5,'0')}`),{revision,index:offset+index,data}));await batch.commit();}
async function writeChunks(uid:string,revision:string,chunks:string[]):Promise<void>{const offsets:Array<number>=[];for(let offset=0;offset<chunks.length;offset+=WRITE_BATCH_CHUNKS)offsets.push(offset);let cursor=0;const worker=async()=>{while(true){const slot=cursor++;if(slot>=offsets.length)return;await writeChunkBatch(uid,revision,chunks,offsets[slot]??0);}};await Promise.all(Array.from({length:Math.min(WRITE_BATCH_CONCURRENCY,offsets.length)},()=>worker()));}
async function cleanupRevision(uid:string,revision:string,count:number):Promise<void>{if(!revision||count<=0||count>MAX_CHUNKS)return;const col=vaultCollection(uid);for(let offset=0;offset<count;offset+=200){const batch=db().batch();for(let i=offset;i<Math.min(count,offset+200);i++)batch.delete(col.doc(`chunk-${revision}-${String(i).padStart(5,'0')}`));try{await batch.commit();}catch{}}}
...
async function pruneCloudHistory(uid:string):Promise<void>{
  try{
    const snap=await historyCollection(uid).orderBy('updatedAt','desc').get();
    const docs=snap.docs||[];
    const keep=new Set<string>();
    docs.slice(0,HISTORY_LIMIT).forEach((doc:any)=>keep.add(String(doc.id)));
    const cutoff=Date.now()-HISTORY_DAILY_DAYS*24*60*60*1000;
    const keptDays=new Set<string>();
    for(const doc of docs){
      const data=doc.data();if(!validMeta(data))continue;
      const ms=Date.parse(data.updatedAt);if(!Number.isFinite(ms)||ms<cutoff)continue;
      const day=new Date(ms).toISOString().slice(0,10);
      if(!keptDays.has(day)){keptDays.add(day);keep.add(String(doc.id));}
    }
...
async function commitMetaIfUnchanged(uid:string,meta:CloudVaultMeta,previous:CloudVaultMeta|null):Promise<void>{const ref=vaultCollection(uid).doc('meta');await db().runTransaction(async(transaction:any)=>{const snap=await transaction.get(ref);if(previous){if(!snap.exists)throw new Error('Cloud changed during sync. Nothing was overwritten. Sync again to compare both copies.');const current=snap.data();if(!validMeta(current)||current.revision!==previous.revision||current.cipherSha256!==previous.cipherSha256)throw new Error('Cloud changed during sync. Nothing was overwritten. Sync again to compare both copies.');}else if(snap.exists)throw new Error('Cloud changed during sync. Nothing was overwritten. Sync again to compare both copies.');transaction.set(ref,meta);});}
async function commitSingleChunkIfUnchanged(uid:string,meta:CloudVaultMeta,previous:CloudVaultMeta|null,cipher:string):Promise<void>{
  ...
  await db().runTransaction(async(transaction:any)=>{const snap=await transaction.get(metaRef);if(previous){if(!snap.exists)throw new Error('Cloud changed during sync. Nothing was overwritten. Sync again to compare both copies.');const current=snap.data();if(!validMeta(current)||current.revision!==previous.revision||current.cipherSha256!==previous.cipherSha256)throw new Error('Cloud changed during sync. Nothing was overwritten. Sync again to compare both copies.');}else if(snap.exists)throw new Error('Cloud changed during sync. Nothing was overwritten. Sync again to compare both copies.');transaction.set(chunkRef,{revision:meta.revision,index:0,data:cipher});transaction.set(metaRef,meta);});
}
async function publishVault(uid:string,security:SecurityMetadata,vault:EncryptedVaultRecord,previous:CloudVaultMeta|null):Promise<CloudVaultMeta>{
  ...
  try{if(chunks.length===1)await commitSingleChunkIfUnchanged(uid,meta,previous,chunks[0]??'');else{await writeChunks(uid,revision,chunks);await commitMetaIfUnchanged(uid,meta,previous);}}catch(error){if(chunks.length>1)await cleanupRevision(uid,revision,chunks.length);throw error;}
...
export async function pushLocalVaultToCloud(uid:string,localSnapshot?:EncryptedVaultRecord|null):Promise<void>{
  ...
  if(previous&&previous.cipherSha256===cipherSha256){writeSyncAnchor(uid,previous);return;}
  if(previous){const anchor=readSyncAnchor(uid);if(!anchor){writeConflict(uid,cipherSha256,previous);throw new Error('This device and your LOUREX account contain different data and this device has no verified sync checkpoint. Nothing was overwritten. Choose which copy to keep in Cloud Sync.');}const remoteChanged=previous.revision!==anchor.revision||previous.cipherSha256!==anchor.cipherSha256;if(remoteChanged){writeConflict(uid,cipherSha256,previous);throw new Error('Your LOUREX account changed on another device. This device was prevented from overwriting it. Both copies are safe; choose which copy to keep in Cloud Sync.');}}
  await publishVault(uid,security,vault,previous);
}

async function pullCloudVaultFromMeta(uid:string,meta:CloudVaultMeta):Promise<{security:SecurityMetadata;vault:EncryptedVaultRecord}>{const col=vaultCollection(uid);const chunks:string[]=[];for(let offset=0;offset<meta.chunkCount;offset+=24){const snaps=await Promise.all(Array.from({length:Math.min(24,meta.chunkCount-offset)},(_,j)=>{const i=offset+j;return col.doc(`chunk-${meta.revision}-${String(i).padStart(5,'0')}`).get();}));snaps.forEach((snap:any,j:number)=>{const i=offset+j;if(!snap.exists)throw new Error(`Cloud vault chunk ${i+1} is missing.`);const data=snap.data();if(data?.revision!==meta.revision||data?.index!==i||typeof data?.data!=='string'||data.data.length>CHUNK_SIZE)throw new Error('Cloud vault is incomplete.');chunks.push(data.data as string);});}const cipher=chunks.join('');if(cipher.length!==meta.cipherLength||await sha256(cipher)!==meta.cipherSha256)throw new Error('Cloud vault integrity check failed.');return {security:meta.security,vault:{id:'vault',schemaVersion:meta.schemaVersion,iv:meta.iv,cipher,updatedAt:meta.updatedAt}};}
...
export async function installCloudVault(uid:string):Promise<boolean>{const meta=await getCloudVaultMeta(uid);if(!meta)return false;const remote=await pullCloudVaultFromMeta(uid,meta);await createSafetySnapshot('pre-restore');await putSecurityAndVault(remote.security,remote.vault);writeSyncAnchor(uid,meta);clearConflict(uid);return true;}
...
export async function resolveCloudConflictWithCloud(uid:string):Promise<void>{requireCurrentUid(uid);if(!readConflict(uid))throw new Error('There is no cloud conflict to resolve.');const installed=await installCloudVault(uid);if(!installed)throw new Error('The cloud copy is no longer available.');clearConflict(uid);}
...
export async function reconcileCloudVault(uid:string):Promise<CloudSyncResult>{
  ...
  if(local&&!remote){clearConflict(uid);if(startup){window.setTimeout(()=>void pushLocalVaultToCloud(uid).catch(()=>undefined),1500);return 'same';}await pushLocalVaultToCloud(uid);return 'pushed';}
  ...
  if(anchor){
    ...
    if(localChanged){if(startup){window.setTimeout(()=>void pushLocalVaultToCloud(uid).catch(()=>undefined),1500);return 'same';}await pushLocalVaultToCloud(uid);return 'pushed';}
    writeSyncAnchor(uid,remote);return 'same';
  }
  writeConflict(uid,localHash,remote);
  throw new Error('Cloud safety check found different local and account data with no verified sync checkpoint. Nothing was overwritten. Both copies are safe; choose which copy to keep in Cloud Sync.');
}
