...
import { buildAiBusinessContext, type AiBusinessContext } from '../lib/ai-business.js';
...
export interface AiItemProposal {
  capability:'item.archive'|'item.restore'|'item.updateMetadata'|'item.reviewDuplicate';
  itemId:string;
  relatedItemId:string;
  label:string;
  rationale:string;
  patch?:{sku?:string;descriptionEn?:string;descriptionAr?:string;hsCode?:string;origin?:string;packing?:string;unit?:string;category?:string;tags?:string[]};
}
export interface AiDocumentDraftProposal {
  capability:'document.createDraft';
  kind:'proforma'|'invoice';
  customerId:string;
  currency:string;
  language:DocumentLanguage;
  items:Array<{savedItemId:string;descriptionEn:string;descriptionAr:string;quantity:string;unit:string;unitPrice:string}>;
  incoterm:string;
  paymentTerms:string;
  deliveryTime:string;
  remarks:string;
...
interface AiServerResponse {answer:string;proposal?:AiProposal|null;}
...
function archived(item:SavedItem):boolean{return Boolean((item as SavedItem&{archived?:boolean}).archived);}

function draftReference(vault:VaultPayload,message:string):DraftReference{
  const q=normalized(message);const tokens=q.split(' ').filter(token=>token.length>=2);
  const score=(text:string)=>{const value=normalized(text);return tokens.reduce((total,token)=>total+(value.includes(token)?1:0),0);};
  ...
  const items=vault.savedItems.filter(item=>!archived(item)).map(item=>({item,score:score([item.sku??'',item.descriptionEn,item.descriptionAr,item.hsCode,item.category??'',...(item.tags??[])].join(' '))})).sort((a,b)=>b.score-a.score||b.item.updatedAt.localeCompare(a.item.updatedAt)).slice(0,18).map(({item})=>({id:item.id,name:(item.descriptionEn||item.descriptionAr||item.sku||'Item').trim(),sku:item.sku??'',unit:item.unit||'PCS',lastUnitPrice:item.lastUnitPrice||'',lastCurrency:item.lastCurrency||''}));
  const smart=vault.appSettings.smartDefaults;
  return{customers,items,defaults:{currency:smart.currency||vault.company.defaultCurrency||'USD',language:smart.language||vault.company.defaultLanguage||'en',incoterm:smart.incoterm||vault.company.defaultIncoterm||'',paymentTerms:smart.paymentTerms||vault.company.defaultPaymentTerms||'',deliveryTime:smart.deliveryTime||vault.company.defaultDeliveryTime||''}};
}

export function buildAiContext(screen:AiWorkspaceScreen,language:UiLanguage,financeSource:AiFinanceSource,vault:VaultPayload,message:string):AiContextEnvelope{
  return {version:3,screen,language,allowedCapabilities:AI_CAPABILITIES.map(capability=>capability.id),finance:buildAiFinanceContext(financeSource,message),business:buildAiBusinessContext(vault),drafting:draftReference(vault,message)};
}
...
function safeMetadataPatch(value:any):AiItemProposal['patch']{
  ...
  if(Array.isArray(value.tags))patch.tags=Array.from(new Set(value.tags.map((tag:any)=>bounded(tag,40)).filter(Boolean))).slice(0,12);
...
export function safeProposal(value:any,context:AiContextEnvelope):AiProposal|null{
  ...
  if(['item.archive','item.restore','item.updateMetadata','item.reviewDuplicate'].includes(value.capability)&&productIds.has(String(value.itemId||''))){
    const capability=value.capability as AiItemProposal['capability'];
    const relatedItemId=productIds.has(String(value.relatedItemId||''))?String(value.relatedItemId):'';
    const patch=capability==='item.updateMetadata'?safeMetadataPatch(value.patch):undefined;
    if(capability==='item.updateMetadata'&&!patch)return null;
    return{capability,itemId:String(value.itemId),relatedItemId,label:bounded(value.label||'Review product action',80),rationale:bounded(value.rationale,220),patch};
  }
  if(value.capability==='document.createDraft'){
    const kind=value.kind==='invoice'?'invoice':value.kind==='proforma'?'proforma':null;if(!kind)return null;
    const customerIds=new Set(context.drafting.customers.map(customer=>customer.id));const itemIds=new Set(context.drafting.items.map(item=>item.id));
    const currency=bounded(value.currency||context.drafting.defaults.currency,8).toUpperCase();if(!/^[A-Z0-9]{2,8}$/.test(currency))return null;
    const language:DocumentLanguage=value.language==='ar'||value.language==='bilingual'?value.language:'en';
...
function starterPrompts(screen:AiWorkspaceScreen):string[]{
  if(screen==='home')return [t('Give me today’s business brief','أعطني ملخص الأعمال اليوم'),t('Create a quotation draft','أنشئ مسودة عرض سعر')];
  ...
  if(screen==='receivables')return [t('Who needs collection follow-up first?','من يحتاج متابعة تحصيل أولًا؟'),t('Draft a collection message for an overdue customer','جهز رسالة تحصيل لعميل متأخر')];
  if(screen==='customers')return [t('Explain customer payment behavior','اشرح سلوك دفع العملاء'),t('Show customers with credit risk signals','اعرض العملاء مع مؤشرات مخاطر ائتمانية')];
  if(screen==='items')return [t('Show duplicate or dormant products','اعرض الأصناف المكررة أو الخاملة'),t('Show pricing and cost anomalies','اعرض شذوذ الأسعار والتكاليف')];
  if(screen==='operations')return [t('Compare supplier costs by product','قارن تكاليف الموردين حسب الصنف'),t('Show recent landed-cost increases','اعرض ارتفاعات تكلفة الوصول الأخيرة')];
  if(screen==='editor')return [t('Review this document before finalizing','راجع هذا المستند قبل الاعتماد'),t('Explain this document financially','اشرح هذا المستند ماليًا')];
...
export class AiCopilot extends React.Component<Props,State>{
  ...
  private ask=async(raw?:string)=>{
    ...
    const capability=capabilityFor(message);const userMessage:AiMessage={id:id('user'),role:'user',text:message};
    this.setState(state=>({busy:true,error:'',input:'',proposal:null,messages:[...state.messages,userMessage]}));this.addAudit(capability,'requested');
    try{
      const resumed=await resumeVaultSession();if(!resumed)throw new Error(t('Unlock LOUREX before using LOUREX AI.','افتح قفل LOUREX قبل استخدام ذكاء LOUREX.'));
      ...
      const context=buildAiContext(this.props.screen,this.props.language,financeSource,resumed.vault,message);
      ...
      const proposal=safeProposal(payload?.proposal,context);const assistant:AiMessage={id:id('assistant'),role:'assistant',text:answer};
      this.setState(state=>({busy:false,messages:[...state.messages,assistant],proposal}));this.addAudit(capability,'answered');
  ...
  private executeItemProposal=async(proposal:AiItemProposal)=>{
    ...
    const resumed=await resumeVaultSession();if(!resumed)throw new Error(t('Unlock LOUREX first.','افتح قفل LOUREX أولًا.'));
    const index=resumed.vault.savedItems.findIndex(item=>item.id===proposal.itemId);if(index<0)throw new Error(t('Product no longer exists.','الصنف لم يعد موجودًا.'));
    const current=resumed.vault.savedItems[index]!;let updated:SavedItem;
    if(proposal.capability==='item.archive')updated={...current,archived:true,updatedAt:new Date().toISOString()} as SavedItem;
    else if(proposal.capability==='item.restore')updated={...current,archived:false,updatedAt:new Date().toISOString()} as SavedItem;
    else{
      const patch=proposal.patch??{};updated={...current,...patch,tags:patch.tags??current.tags,updatedAt:new Date().toISOString()} as SavedItem;
      const duplicate=findSavedItemDuplicate(resumed.vault.savedItems,updated);if(duplicate)throw new Error(t('This metadata would create a duplicate product. Review it in Items instead.','هذه البيانات ستنشئ صنفًا مكررًا. راجع الصنف في قسم الأصناف بدلًا من ذلك.'));
    }
  ...
  private executeDocumentProposal=async(proposal:AiDocumentDraftProposal)=>{
    const resumed=await resumeVaultSession();if(!resumed)throw new Error(t('Unlock LOUREX first.','افتح قفل LOUREX أولًا.'));
    const next=nextDocumentNumber(resumed.vault,proposal.kind);let doc=createBlankDocument(proposal.kind,next.number,resumed.vault.company);const customer=resumed.vault.customers.find(item=>item.id===proposal.customerId);
    if(customer)doc=applyCustomerCommercialDefaults({...doc,customerSnapshot:customerSnapshotFrom(customer)},customer,resumed.vault.company);
    doc={...doc,currency:proposal.currency,language:proposal.language,terms:{...doc.terms,incoterm:proposal.incoterm||doc.terms.incoterm,paymentTerms:proposal.paymentTerms||doc.terms.paymentTerms,deliveryTime:proposal.deliveryTime||doc.terms.deliveryTime,remarks:proposal.remarks||doc.terms.remarks},notes:proposal.notes||doc.notes};
    doc.items=proposal.items.map(entry=>{
      const saved=resumed.vault.savedItems.find(item=>item.id===entry.savedItemId&&!archived(item));
      if(saved){const line=documentItemFromSavedItem(saved);line.quantity=entry.quantity;line.unit=entry.unit||line.unit;if(entry.unitPrice)line.unitPrice=entry.unitPrice;else if(saved.lastCurrency&&saved.lastCurrency!==proposal.currency)line.unitPrice='';return line;}
      return{id:makeId('item'),descriptionEn:entry.descriptionEn,descriptionAr:entry.descriptionAr,hsCode:'',origin:'',packing:'',quantity:entry.quantity,unit:entry.unit||'PCS',unitPrice:entry.unitPrice,unitCost:''};
    });
    const now=new Date().toISOString();doc={...doc,status:'draft',createdAt:now,updatedAt:now};
    const vault={...next.vault,documents:[...next.vault.documents,doc],documentEvents:[...next.vault.documentEvents,createDocumentEvent(doc,'created')]};await saveVault(resumed.key,vault);window.location.reload();
  };

  private approveProposal=async()=>{
    const proposal=this.state.proposal;if(!proposal||this.state.busy)return;
    if(!capabilityRequiresApproval(proposal.capability))return;
    this.addAudit(proposal.capability,'approved');this.setState({busy:true,error:''});
    try{
      if(proposal.capability==='workspace.navigate'){this.setState({busy:false,proposal:null,open:false},()=>this.props.onNavigate(proposal.target));return;}
      if(proposal.capability==='document.createDraft')await this.executeDocumentProposal(proposal);else await this.executeItemProposal(proposal);
    }catch(error){const text=error instanceof Error?error.message:t('Unable to apply this AI action.','تعذر تطبيق إجراء الذكاء.');this.setState({busy:false,error:text});this.addAudit(proposal.capability,'failed');}
  };
  ...
  private proposalPreview=(proposal:AiProposal):string=>{
    if(proposal.capability==='workspace.navigate')return `${t('Destination','الوجهة')}: ${proposal.target}`;
    if(proposal.capability==='document.createDraft')return `${t('Type','النوع')}: ${proposal.kind==='invoice'?t('Invoice','فاتورة'):t('Quotation','عرض سعر')}\n${t('Currency','العملة')}: ${proposal.currency}\n${t('Items','الأصناف')}: ${proposal.items.length}${proposal.customerId?`\n${t('Customer linked','العميل مرتبط')}`:''}`;
    if(proposal.capability==='item.updateMetadata')return Object.entries(proposal.patch??{}).map(([key,value])=>`${key}: ${Array.isArray(value)?value.join(', '):value}`).join('\n');
    return proposal.relatedItemId?`${t('Related product','الصنف المرتبط')}: ${proposal.relatedItemId}`:'';
  };

  render():any{
    const prompts=starterPrompts(this.props.screen);const preview=this.state.proposal?this.proposalPreview(this.state.proposal):'';
    return <>
      ...
      {this.state.open?<>
        ...
        <aside id="lourex-ai-panel" className="lourex-ai-panel" role="dialog" aria-modal="true" aria-label={t('LOUREX AI','ذكاء LOUREX')} dir={this.props.language==='ar'?'rtl':'ltr'}>
          ...
          <div className="lourex-ai-messages" aria-live="polite">
            {!this.state.messages.length?<div className="lourex-ai-empty"><strong>{t('Ask about the business or prepare a safe action','اسأل عن الأعمال أو جهّز إجراءً آمنًا')}</strong><p>{t('LOUREX calculates accounting and intelligence locally. AI explains the results and may prepare a preview; any record change requires your approval.','LOUREX يحسب المحاسبة والمؤشرات محليًا. الذكاء يشرح النتائج وقد يجهز معاينة؛ أي تغيير على السجلات يحتاج موافقتك.')}</p><div className="lourex-ai-starters">{prompts.map(prompt=><button type="button" key={prompt} onClick={()=>void this.ask(prompt)}>{prompt}</button>)}</div></div>:null}
            {this.state.messages.map(message=><div key={message.id} className={`lourex-ai-message ${message.role}`}>{message.text}</div>)}
            {this.state.busy?<div className="lourex-ai-busy">{t('LOUREX AI is working…','ذكاء LOUREX يعمل…')}</div>:null}
          </div>
