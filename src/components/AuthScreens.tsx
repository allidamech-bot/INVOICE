...
interface SetupState { welcome: boolean; step: 1|2|3; pin: string; confirm: string; company: CompanySettings; error: string; busy: boolean; restoreFile:File|null; restorePin:string; restoreBusy:boolean; restoreOpen:boolean; }

export class SetupScreen extends React.Component<SetupProps, SetupState> {
  state: SetupState = { welcome: true, step: 1, pin: '', confirm: '', company: this.props.initialCompany, error: '', busy: false, restoreFile:null, restorePin:'', restoreBusy:false, restoreOpen:false };
  ...
  render(): any {
    ...
    if (this.state.welcome) return <div className="auth-page"><div className="auth-card unlock-card welcome-card">{this.languageSwitch()}<Brand logoDataUrl={this.props.logoDataUrl} language={this.props.language}/><p className="eyebrow">LOUREX Invoice</p><h1>{t('Welcome.','مرحبًا.')}</h1><p className="subtle">{t('Create professional invoices with secure cloud sync and offline access.','أنشئ فواتير احترافية مع مزامنة سحابية آمنة وإمكانية العمل دون اتصال.')}</p><Button className="welcome-primary" variant="primary" onClick={()=>this.setState({welcome:false,error:''})}>{t('Create New Account','إنشاء حساب جديد')}</Button><div className="welcome-account-divider"><span>{t('Already have your data?','لديك حساب أو بيانات سابقة؟')}</span></div><div className="welcome-secondary-grid"><button type="button" className="welcome-secondary-action cloud-entry" onClick={this.openCloud}>{t('Sign In to Cloud','تسجيل الدخول السحابي')}</button><button type="button" className="welcome-secondary-action restore-entry" onClick={()=>this.setState({restoreOpen:!this.state.restoreOpen,error:''})}>{t('Restore Backup','استعادة نسخة احتياطية')}</button></div>{this.state.restoreOpen?<div className="welcome-restore-panel"><label className="file-picker"><strong>{this.state.restoreFile?this.state.restoreFile.name:t('Choose Backup File','اختر ملف النسخة الاحتياطية')}</strong><input type="file" accept=".lourex-backup,application/json,.json" onChange={(e:any)=>this.setState({restoreFile:e.target.files?.[0]??null,error:''})}/></label>{this.state.restoreFile?<div className="restore-first-run"><Field label={t('Backup PIN','رمز PIN للنسخة الاحتياطية')}><Input inputMode="numeric" type="password" maxLength="12" value={this.state.restorePin} onChange={(e:any)=>this.setState({restorePin:e.target.value.replace(/\D/g,''),error:''})}/></Field><Button disabled={this.state.restoreBusy} onClick={()=>void this.restoreExisting()}>{this.state.restoreBusy?t('Restoring…','جارٍ الاستعادة…'):t('Restore','استعادة')}</Button></div>:null}</div>:null}{error?<div className="auth-error">{error}</div>:null}<p className="security-note">{t('Encrypted · Cloud sync · Works offline','مشفّر · مزامنة سحابية · يعمل دون اتصال')}</p></div></div>;
