...
export class SetupScreen extends React.Component<SetupProps, SetupState> {
  ...
  render(): any {
    const { company, busy, error } = this.state;
...
export class UnlockScreen extends React.Component<UnlockProps, UnlockState> {
  ...
  private open=async():Promise<void>=>{
    this.setState({busy:true,error:''});
    try{await this.props.onUnlock('');}
    catch(err){this.setState({busy:false,error:err instanceof Error?err.message:t('Unable to open account data.','تعذر فتح بيانات الحساب.')});}
  };
