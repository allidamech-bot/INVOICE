...
export class ReportsPage extends React.Component<Props,State>{
  ...
  render():any{
    ...
    // A date-range change can remove the previously selected currency from the
    // report. Fall back to the truthful "all currencies" view instead of
    // keeping an invisible stale value that makes every report section blank.
    ...
    return <div className="reports-page financial-report-print">
      ...
      <section className="reports-filter-panel" aria-label={t('Report filters','فلاتر التقرير')}>
        ...
        <label><span>{t('Currency','العملة')}</span><Select value={selected||'ALL'} onChange={(e:any)=>this.setState({currency:e.target.value})}><option value="ALL">{t('All currencies — separate','كل العملات — منفصلة')}</option>{currencies.map(currency=><option key={currency} value={currency}>{currency}</option>)}</Select></label>
      </section>

      <div className="report-print-header"><div className="report-print-brand"><img src={logo} alt={companyDisplayName(this.props.company)}/><div><strong>{companyDisplayName(this.props.company)}</strong><span>{t('Financial Management Report','تقرير الإدارة المالية')}</span></div></div><div><strong>{reportTitle}</strong><span>{selected||t('Currencies shown separately','العملات معروضة بشكل منفصل')}</span></div></div>
