...
export function ProductsInventoryWorkspace(props:Props):any{
  ...
  return <section className="domain-workspace products-inventory-workspace">
    <DomainWorkspaceTabs value={tab} onChange={(value)=>{setTab(value);if(value==='inventory')setFocusItemId('');}} ariaLabel={t('Products and inventory sections','أقسام المنتجات والمخزون')} options={[
    ...
    {tab==='products'?<SavedItemsPage items={props.items} currency={props.currency} onSave={props.onSaveItem} onSaveMany={props.onSaveItems} onDelete={props.onDeleteItem} onInspectInventory={(item)=>{setFocusItemId(item.id);setTab('movements');}} onInspectPurchases={(item)=>setHistoryItem(item)}/>:null}
