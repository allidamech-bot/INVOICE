...
export class SettingsModal extends React.Component<Props,State> {
  ...
  private securitySettings(s:AppSettings,account:CloudUser|null):any{return <div className="settings-tab-page security-settings-page">
    <div className="settings-title"><div><p className="eyebrow">{t('Security','الأمان')}</p><h3>{t('Security & recovery','الأمان والاستعادة')}</h3><p>{t('Session locking, device PIN and encrypted cloud recovery. Sign out is available directly from Settings.','قفل الجلسة ورمز PIN والاستعادة السحابية المشفّرة. تسجيل الخروج متاح مباشرة من الإعدادات.')}</p></div></div>
