...
export class SettingsModal extends React.Component<Props,State> {
  ...
  private documentSettings(c:CompanySettings,s:AppSettings):any{return <div className="settings-tab-page document-settings-page">
    ...
    <section className="settings-section"><div className="settings-section-heading"><div><h4>{t('Document defaults','الإعدادات الافتراضية للمستند')}</h4><p>{t('Defaults used when a new quotation, invoice or purchase order is created. Commercial terms are managed under Commercial.','إعدادات تُستخدم عند إنشاء عرض سعر أو فاتورة أو طلب شراء جديد. تتم إدارة الشروط التجارية ضمن «تجاري».')}</p></div>{this.saveButton('company')}</div><div className="form-grid two">
  ...
  private securitySettings(s:AppSettings,account:CloudUser|null):any{return <div className="settings-tab-page security-settings-page">
    <div className="settings-title"><div><p className="eyebrow">{t('Security','الأمان')}</p><h3>{t('Security & recovery','الأمان والاستعادة')}</h3><p>{t('Session locking, device PIN and encrypted cloud recovery. Sign out is available from More.','قفل الجلسة ورمز PIN والاستعادة السحابية المشفّرة. تسجيل الخروج متاح من صفحة المزيد.')}</p></div></div>
