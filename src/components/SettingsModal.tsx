...
import { currentCloudUser, installCloudVault } from '../cloud/firebase.js';
...
interface State {
  ...
  savedSection:'company'|'documents'|null; currentPin:string; newPin:string; confirmPin:string; confirmClose:boolean;
...
export class SettingsModal extends React.Component<Props,State> {
  constructor(props:Props){
    ...
    this.state={tab:'company',company,appSettings,busy:false,cleaningAssets:false,message:'',error:'',savedSection:null,currentPin:'',newPin:'',confirmPin:'',confirmClose:false,companyInitial:JSON.stringify(company),documentsInitial:JSON.stringify(appSettings),logoOriginalDataUrl:'',logoCleanedDataUrl:'',logoRebuiltDataUrl:'',logoMode:'auto'};
  }
  ...
  private restoreFromCloud=async()=>{
    ...
    }catch(e){this.setState({busy:false,error:e instanceof Error?e.message:t('Unable to restore account data.','تعذر استرجاع بيانات الحساب.')});}
  };
  ...
  render():any{
    const c=this.state.company,s=this.state.appSettings;const hasCompanyLogo=Boolean(c.logoDataUrl&&!c.logoDataUrl.includes('lourex-logo.svg'));const account=currentCloudUser();
    return <Modal open={this.props.open} title={t('Settings','الإعدادات')} size="xl" onClose={this.requestClose}><div className="settings-layout"><nav className="settings-tabs">{([['company',t('Company','الشركة')],['commercial',t('Commercial','تجاري')],['documents',t('Documents','المستندات')],['security',t('Security','الأمان')]] as const).map(([id,label])=><button type="button" key={id} className={this.state.tab===id?'active':''} onClick={()=>this.setState({tab:id,error:'',message:'',savedSection:null})}>{label}</button>)}</nav><div className="settings-panel">
    ...
    {this.state.tab==='security'?<div><div className="settings-title"><div><p className="eyebrow">{t('Security','الأمان')}</p><h3>{t('Account access','الدخول إلى الحساب')}</h3></div></div><div className="settings-section cloud-restore-only"><h4>{t('Cloud account data','بيانات الحساب السحابية')}</h4><p className="settings-note">{account?t(`Signed in as ${account.email}. Data is saved automatically to this account.`,`تم تسجيل الدخول باسم ${account.email}. يتم حفظ البيانات تلقائيًا في هذا الحساب.`):t('Sign in to your LOUREX account to load its data.','سجّل الدخول إلى حساب LOUREX لتحميل بياناته.')}</p><Button icon="restore" variant="primary" disabled={this.state.busy||!account} onClick={()=>void this.restoreFromCloud()}>{this.state.busy?t('Restoring…','جارٍ الاسترجاع…'):t('Restore from Cloud','استرجاع من السحابة')}</Button><p className="settings-note">{t('This loads the latest data stored for the signed-in email. No backup file, backup PIN, or extra password is required.','يتم تحميل آخر بيانات محفوظة للإيميل المسجل حاليًا. لا يحتاج ملف نسخة احتياطية أو رمز PIN للنسخة أو كلمة مرور إضافية.')}</p></div><div className="settings-section"><h4>{t('Auto Lock','القفل التلقائي')}</h4><Field label={t('Lock after inactivity','القفل بعد عدم النشاط')}><Select value={String(s.autoLockMinutes)} onChange={(e:any)=>void this.changeAutoLock(Number(e.target.value) as AutoLockMinutes)}><option value="0">{t('Never','أبدًا')}</option><option value="5">{t('5 minutes','5 دقائق')}</option><option value="15">{t('15 minutes','15 دقيقة')}</option><option value="30">{t('30 minutes','30 دقيقة')}</option></Select></Field><Button icon="lock" onClick={this.props.onLock}>{t('Lock App','قفل التطبيق')}</Button></div><div className="settings-section"><h4>{t('Change PIN','تغيير رمز PIN')}</h4><div className="form-grid one"><Field label={t('Current PIN','رمز PIN الحالي')}><Input inputMode="numeric" type="password" value={this.state.currentPin} onChange={(e:any)=>this.setState({currentPin:e.target.value.replace(/\D/g,'')})}/></Field><Field label={t('New PIN','رمز PIN الجديد')}><Input inputMode="numeric" type="password" value={this.state.newPin} onChange={(e:any)=>this.setState({newPin:e.target.value.replace(/\D/g,'')})}/></Field><Field label={t('Confirm New PIN','تأكيد رمز PIN الجديد')}><Input inputMode="numeric" type="password" value={this.state.confirmPin} onChange={(e:any)=>this.setState({confirmPin:e.target.value.replace(/\D/g,'')})}/></Field></div><Button variant="primary" disabled={this.state.busy} onClick={this.changePin}>{t('Change PIN','تغيير رمز PIN')}</Button></div></div>:null}
