...
interface State{open:boolean;query:string;}
...
export class GlobalSearch extends React.Component<Props,State>{
  ...
  private handleKeyDown=(event:KeyboardEvent)=>{
    if(event.key==='Escape'&&this.state.open){event.preventDefault();this.close();return;}
    if(event.key.toLowerCase()==='k'&&(event.metaKey||event.ctrlKey)){
      event.preventDefault();
      this.state.open?this.close():this.open();
    }
  };
  private open=()=>this.setState({open:true,query:''},()=>window.setTimeout(()=>this.inputRef?.focus(),0));
  private close=()=>this.setState({open:false,query:''});
  ...
  render():any{
    ...
    return <><button type="button" className="global-search-backdrop" aria-label={t('Close global search','إغلاق البحث الشامل')} onClick={this.close}/><section className="global-search-panel" role="dialog" aria-modal="true" aria-label={t('Search LOUREX','بحث LOUREX')} dir={this.props.language==='ar'?'rtl':'ltr'}>
      <header className="global-search-input-wrap"><Icon name="search"/><input ref={(node:HTMLInputElement|null)=>{this.inputRef=node;}} value={this.state.query} onChange={(event:any)=>this.setState({query:event.target.value})} placeholder={t('Search documents, customers, products, suppliers or purchases…','ابحث في المستندات والعملاء والمنتجات والموردين والمشتريات…')} aria-label={t('Search LOUREX','بحث LOUREX')}/><kbd>ESC</kbd></header>
      {!this.state.query.trim()?<div className="global-search-start">
        <div className="global-search-section-title"><span>{t('Quick create','إنشاء سريع')}</span><small>{t('Creates in the canonical workspace','ينشئ في مساحة العمل الأصلية')}</small></div>
        <div className="global-search-actions"><button type="button" onClick={()=>this.create('proforma')}><Icon name="proforma"/><span><strong>{t('New quotation','عرض سعر جديد')}</strong><small>{t('Create a commercial quotation','إنشاء عرض تجاري')}</small></span></button><button type="button" onClick={()=>this.create('invoice')}><Icon name="invoice"/><span><strong>{t('New invoice','فاتورة جديدة')}</strong><small>{t('Create an invoice draft','إنشاء مسودة فاتورة')}</small></span></button></div>
