...
import { customerMatchesSearch } from '../lib/customer-search.js';
...
import { DocumentAttachmentsSection } from './DocumentAttachmentsSection.js';
...
export class EditorPage extends React.Component<Props,State>{
  ...
  private issueDate=(value:string)=>this.mutate(d=>{
    ...
    if(customer&&/^\d+$/.test(customer.paymentDueDays.trim()))next={...next,dueDate:addDaysIso(value,Math.min(3650,Number(customer.paymentDueDays)))};
    return next;
  });
  private applyTermPreset=(preset:PaymentTermPreset)=>this.mutate(d=>applyPaymentTermPreset(d,preset));
  private selectBank=(id:string)=>{const bank=bankDetailsForId(this.props.company,id);if(!bank)return;this.mutate(d=>({...d,bankAccountId:id,companySnapshot:{...d.companySnapshot,bank:{...bank}}}));};
  ...
  private saveDocumentItem=async(item:DocumentItem)=>{
    ...
    try{await this.props.onSaveDocumentItem(item,this.state.doc.currency);}catch(e){this.setGlobalError(e instanceof Error?e.message:t('Unable to save item.','تعذر حفظ الصنف.'));}
  };
  private saveTemplateDefaults=async(smart:AppSettings['smartDefaults'])=>{try{await this.props.onSaveSmartDefaults(smart);}catch(e){this.setGlobalError(e instanceof Error?e.message:t('Unable to save template preference.','تعذر حفظ تفضيل القالب.'));}};
  private toggleFavoriteTemplate=(id:TemplateId)=>{const current=this.props.smartDefaults.favoriteTemplateIds;const favoriteTemplateIds=current.includes(id)?current.filter(item=>item!==id):[id,...current];void this.saveTemplateDefaults({...this.props.smartDefaults,favoriteTemplateIds});};
  ...
  render():any{
    ...
    const groupLabel=(key:string)=>key==='document'?t('Document','المستند'):key==='customer'?(isSupplierDocument?t('Supplier','المورد'):t('Customer','العميل')):key==='items'?t('Items','الأصناف'):t('Pricing','التسعير');
    ...
    const defaultTemplateId=(d.kind==='proforma'||d.kind==='proforma-invoice'||d.kind==='rfq')?this.props.smartDefaults.quoteTemplateId:d.kind==='invoice'?this.props.smartDefaults.invoiceTemplateId:d.appearance.templateId;
    ...
    const hsCodeSuggestions=priorHsCodes(this.props.savedItems,this.props.documents);
    const bankOptions=bankAccountsForCompany(this.props.company);
    ...
    const credit=isSupplierDocument||d.kind!=='invoice'?null:customerCreditStatus(d,this.props.customers,this.props.documents,this.props.payments);
    ...
    return <div className={`editor-screen workflow-${workflow} ${this.state.mobilePreview?'mobile-preview-open':''}`} aria-busy={this.state.issuing}>
      <header className="editor-topbar">
        <div className="editor-top-left"><IconButton icon="arrowLeft" label={t('Back','رجوع')} onClick={()=>void this.saveAndClose()}/><div><strong>{d.number}</strong><span>{t(kindLabel.en,kindLabel.ar)}</span></div><span className={`editor-workflow-badge status-${workflow}`}>{workflow==='final'?t('Final','نهائي'):workflow==='ready'?t('Ready','جاهز'):t('Draft','مسودة')}</span></div>
        <div className="editor-grand-total-chip"><span>{priceOptional?t('Non-financial','غير مالي'):isPurchaseOrder?t('Order Total','إجمالي الطلب'):t('Grand Total','الإجمالي')}</span><strong>{priceOptional?'—':formatMoney(totals.grandTotal,d.currency)}</strong></div>
