...
interface State { query:string; editing:SavedItem|null; editingInitial:string; discardAction:DiscardAction; pendingSelect:SavedItem|null; deleting:SavedItem|null; busy:boolean; error:string; }

function blank(currency:string,seed=''):SavedItem{
  const now=new Date().toISOString();
  const description=seed.trim();
  const arabic=isArabic();
  return {id:makeId('product'),createdAt:now,updatedAt:now,descriptionEn:arabic?'':description,descriptionAr:arabic?description:'',hsCode:'',origin:'',packing:'',unit:'PCS',lastUnitPrice:'',lastCurrency:currency||'USD',usageCount:0,lastUsedAt:now};
}

export class SavedItemsModal extends React.Component<Props,State>{
  state:State={query:'',editing:null,editingInitial:'',discardAction:'',pendingSelect:null,deleting:null,busy:false,error:''};
  componentDidUpdate(prev:Props):void{if(this.props.open&&!prev.open)this.setState({query:'',editing:null,editingInitial:'',discardAction:'',pendingSelect:null,deleting:null,busy:false,error:''});}
  private set=(key:keyof SavedItem,value:any)=>this.setState(state=>({editing:state.editing?{...state.editing,[key]:value}:null,error:''}));
  private beginEdit=(item:SavedItem)=>{const editing=structuredClone(item);this.setState({editing,editingInitial:JSON.stringify(editing),discardAction:'',pendingSelect:null,error:''});};
  ...
  private save=async()=>{
    ...
    try{await this.props.onSave({...item,lastUnitPrice:item.lastUnitPrice.trim()?normalizeDecimalInput(item.lastUnitPrice):'',updatedAt:new Date().toISOString(),lastCurrency});this.setState({busy:false,editing:null,editingInitial:'',discardAction:'',pendingSelect:null,query:''});}
