...
export class SavedItemsModal extends React.Component<Props,State>{
  ...
  private newItem=()=>this.setState({editing:blank(this.props.currency,this.state.query),error:''});
  private save=async()=>{
    ...
    try{await this.props.onSave({...item,lastUnitPrice:item.lastUnitPrice.trim()?normalizeDecimalInput(item.lastUnitPrice):'',updatedAt:new Date().toISOString(),lastCurrency});this.setState({busy:false,editing:null,query:''});}
