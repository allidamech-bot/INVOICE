...
interface State {
  ...
  busy:boolean; error:string; view:LibraryView; category:string; sortMode:SortMode; favoriteBusyId:string;
}
...
export class SavedItemsModal extends React.Component<Props,State>{
  state:State={query:'',editing:null,editingInitial:'',discardAction:'',pendingSelect:null,deleting:null,busy:false,error:'',view:'recent',category:'',sortMode:'smart',favoriteBusyId:''};
  componentDidUpdate(prev:Props):void{
    if(this.props.open&&!prev.open)this.setState({query:'',editing:null,editingInitial:'',discardAction:'',pendingSelect:null,deleting:null,busy:false,error:'',view:this.props.items.some(item=>item.favorite)?'favorites':'recent',category:'',sortMode:'smart',favoriteBusyId:''});
  }
  ...
  private selectItem=(item:SavedItem)=>{if(this.editingDirty()){this.setState({discardAction:'select',pendingSelect:item});return;}this.props.onSelect(item);};
  private confirmDiscard=()=>{
    ...
    if(action==='select'&&pending){this.setState({editing:null,editingInitial:'',discardAction:'',pendingSelect:null,error:''},()=>this.props.onSelect(pending));return;}
  ...
  render():any{
    ...
    return <><Modal open={this.props.open} title={t('Saved Items','الأصناف المحفوظة')} size="xl" onClose={this.requestModalClose}>
      <div className="saved-items-shell">
        <aside className="saved-items-list-pane">
          ...
          <div className="saved-items-list">{filtered.map(item=><article key={item.id} className={`saved-item-row ${item.favorite?'is-favorite':''}`}><button type="button" className={`saved-item-favorite ${item.favorite?'is-active':''}`} aria-pressed={Boolean(item.favorite)} aria-label={item.favorite?t('Remove from favorites','إزالة من المفضلة'):t('Add to favorites','إضافة إلى المفضلة')} disabled={Boolean(this.state.favoriteBusyId)} onClick={()=>void this.toggleFavorite(item)}><span aria-hidden="true">★</span></button><button type="button" className="saved-item-main" onClick={()=>this.selectItem(item)}><strong>{titleOf(item)}</strong>{item.descriptionEn&&item.descriptionAr?<span dir={isArabic()?'ltr':'rtl'}>{isArabic()?item.descriptionEn:item.descriptionAr}</span>:null}<div className="saved-item-row-meta">{categoryOf(item)?<em>{categoryOf(item)}</em>:null}{(item.tags??[]).slice(0,2).map(tag=><em key={tag}>#{tag}</em>)}</div><small>{[item.unit,item.lastUnitPrice?`${item.lastUnitPrice} ${item.lastCurrency}`:'',item.origin].filter(Boolean).join(' · ')}</small></button><IconButton icon="edit" label={t('Edit','تعديل')} onClick={()=>this.beginEdit(item)}/></article>)}</div>
