...
(() => {
  ...
  const ensureInlineOverlay = (mode = pendingMode) => {
    ...
    style.textContent = '.lourex-ios-output-fallback{position:fixed;inset:0;z-index:2147483646;display:grid;place-items:center;padding:max(24px,env(safe-area-inset-top)) max(18px,env(safe-area-inset-right)) max(24px,env(safe-area-inset-bottom)) max(18px,env(safe-area-inset-left));background:rgba(2,4,9,.8);backdrop-filter:blur(18px) saturate(120%);-webkit-backdrop-filter:blur(18px) saturate(120%);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif}.lourex-ios-output-card{width:min(360px,100%);padding:24px;border:1px solid rgba(156,172,255,.2);border-radius:20px;background:#0F1B28;background-image:linear-gradient(145deg,rgba(83,70,216,.11),transparent 38%),linear-gradient(180deg,rgba(255,255,255,.025),transparent);color:#EDF2F7;text-align:center;box-shadow:0 40px 110px rgba(0,0,0,.62),0 1px 0 rgba(255,255,255,.055) inset}.lourex-ios-output-card strong{display:block;font-size:18px;margin:8px 0;color:#F7F5F0}.lourex-ios-output-card span{display:block;color:#9EABBC;font-size:13px;line-height:1.55;margin-bottom:18px}.lourex-ios-output-spinner{width:42px;height:42px;margin:0 auto 14px;border:2px solid #293746;border-top-color:#8C91FF;border-radius:50%;animation:lourexOutputSpin .8s linear infinite}.lourex-ios-output-card button,.lourex-ios-output-card a{min-height:46px;border-radius:12px;border:1px solid #304052;background:#18212D;color:#E9EDF4;padding:0 16px;font:700 14px -apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif}.lourex-ios-output-card a{display:flex;align-items:center;justify-content:center;text-decoration:none}.lourex-ios-output-primary{width:100%;background:#5346D8!important;background-image:linear-gradient(135deg,#6958EA,#456EDD 50%,#2F82C9)!important;border-color:rgba(171,177,255,.46)!important;color:#fff!important;margin-bottom:8px;box-shadow:0 12px 30px rgba(65,72,205,.28)}.lourex-ios-output-secondary{width:100%;margin-bottom:8px}.lourex-ios-output-error{color:#FF9A92!important}@keyframes lourexOutputSpin{to{transform:rotate(360deg)}}@media print{.lourex-ios-output-fallback{display:none!important}}';
