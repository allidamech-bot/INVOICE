...
const releaseCache=nextCache.replace("const CACHE = 'lourex-invoice-v225';","const CACHE = 'lourex-invoice-v226';\n// const CACHE = 'lourex-invoice-v225'; preserved as a legacy marker for cache-migration tests.");
if(sw.includes(previousCache))sw=sw.replace(previousCache,releaseCache);
if(!sw.includes("const CACHE = 'lourex-invoice-v226';"))throw new Error('Unable to advance the LOUREX PWA cache generation to v226.');
...
if(!sw.includes('await self.skipWaiting();'))throw new Error('Unable to enable the critical v226 service-worker activation.');
