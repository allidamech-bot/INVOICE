...
const cacheGenerations=[...sw.matchAll(/^const CACHE = 'lourex-invoice-v(\d+)';$/gm)].map(match=>Number(match[1])).filter(Number.isInteger);
const activeCacheGeneration=Math.max(0,...cacheGenerations);
if(activeCacheGeneration<280)throw new Error('Unable to verify the active LOUREX PWA cache generation.');
