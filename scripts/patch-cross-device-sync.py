...
pattern=r"if\(!linked&&configured\)\{const remote=await getCloudVaultMeta\(user\.uid\);if\(remote\)\{await signOutCloudUser\(\);this\.setState\(\{cloudUser:null,cloudLinked:false\}\);throw new Error\(t\('This cloud account already contains LOUREX data\.[\s\S]*?\}\);\}\}"
app,count=re.subn(pattern,"",app,count=1)
if count!=1: raise SystemExit('attachCloudUser remote blocker not found')
