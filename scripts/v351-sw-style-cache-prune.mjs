...
const activeStylePush=/^\s*LOCAL_CORE\.push\((['"])\.\/styles\/[^'"\r\n]+\.css(?:\?[^'"\r\n]*)?\1\);\s*$/gm;
const retiredPushes=[...sw.matchAll(activeStylePush)].map(match=>match[0].trim());
sw=sw.replace(activeStylePush,'');

const remainingActiveStylePush=/^\s*LOCAL_CORE\.push\((['"])\.\/styles\/[^'"\r\n]+\.css(?:\?[^'"\r\n]*)?\1\);\s*$/m;
if(remainingActiveStylePush.test(sw)){
  throw new Error('v351: an active historical standalone stylesheet push survived the production cache prune.');
}
