...
// v253 — Authenticated gateway transition recovery.
// The account gateway is a child of the already-mounted App. Firebase can make a
// successful email/password session current without causing that parent to render
// again. Safari is especially sensitive to the old 500ms form-owned reload path.
// Watch Firebase itself and, only while the public account gateway is visible,
// perform one UID-scoped reload after the authenticated user is observable. The
// normal startup path then selects that UID's physical IndexedDB before reading
// any encrypted vault/session state.
